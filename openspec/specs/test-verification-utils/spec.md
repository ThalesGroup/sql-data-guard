...
The test suite SHALL verify the behavior of `split_to_expressions` with flattened and non-flattened `sqlglot` expressions.
...
The test suite SHALL verify that `find_direct` correctly identifies and yields direct child expressions of a specified type from a parent expression.
