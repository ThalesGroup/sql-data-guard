...
### Run a Focused Test
```bash
PYTHONPATH=src python -m pytest tests/test_verification_utils.py -k "test_split_to_expressions_matching"
```

### LLM Integration Tests
- Location: `tests/test_sql_guard_llm.py`
...
- Command:
  ```bash
  PYTHONPATH=src python -m pytest --color=yes tests/test_sql_guard_llm.py
  ```
...
- Run/build using FastAPI and configured via `Containerfile`.

### MCP Wrapper
- Located in `src/sql_data_guard/mcpwrapper/mcp_wrapper.py`.
- Managed/packaged via `wrapper.Containerfile`.
